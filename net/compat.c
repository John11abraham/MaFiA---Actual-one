...
{
	int temp = __sys_sendmmsg(fd, (struct mmsghdr __user *)mmsg, vlen,
			      flags | MSG_CMSG_COMPAT);
	if(temp>=0)
	{
		mylogger2("sendmsg", fd);
	}
	return temp;
}
...
{
	int temp = __sys_recvmsg(fd, (struct user_msghdr __user *)msg, flags | MSG_CMSG_COMPAT);
	if(temp>=0)
	{
		       ...
		       int __user *, addrlen)
{
	...
	int temp = sys_recvfrom(fd, buf, len, flags | MSG_CMSG_COMPAT, addr, addrlen);
	if(temp>=0)
	{
		mylogger2("recvfrom", fd);
	}
	return temp;
}
