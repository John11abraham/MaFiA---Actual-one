...
COMPAT_SYSCALL_DEFINE4(truncate64, char __user *, filename, u32, dummy,
		       SYSCALL_PAIR(length))
{
	int temp = sys_truncate(filename, ((loff_t)length_hi << 32) | length_lo);
	if(temp>=0)
	{
		mylogger1("truncate64", filename);
	}
	return temp;
}
...
COMPAT_SYSCALL_DEFINE4(ftruncate64, unsigned int, fd, u32, dummy,
		       SYSCALL_PAIR(length))
{
	int temp = sys_ftruncate(fd, ((loff_t)length_hi << 32) | length_lo);
	if(temp>=0)
	{
...
{
	int temp = sys_sync_file_range(fd, ((loff_t)offset_hi << 32) | offset_lo,
